...
namespace OnPayClient.Models.Extension
{
    ...
    {
        public static AtomicResponse<DetailedTransaction> Authorize(this AtomicResponse<DetailedSubscription> subscription, int amount, string orderId, SurchargeSettings surcharge = null)
        {
            ValidateResponse(subscription);
            return subscription.Data.Authorize(amount, orderId, surcharge);
        }

        public static async Task<AtomicResponse<DetailedTransaction>> AuthorizeAsync(this AtomicResponse<DetailedSubscription> subscription, int amount, string orderId, SurchargeSettings surcharge = null)
        {
            ValidateResponse(subscription);
            return await subscription.Data.AuthorizeAsync(amount, orderId, surcharge);
        }
